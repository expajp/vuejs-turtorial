...
## トランジションの再利用
* transitionやtransition-groupをルート要素にしたコンポーネントを定義することで、トランジションの再利用が可能
    * まあ、子要素と強く結びついたコードになるからこのカタチしかないのは当然
* 関数型コンポーネントはこのタスクに強く適している

## 動的トランジション
* Vueではトランジションもデータドリブンで動かせる
    * 具体的には、特定データを定義することで走るトランジションが定義できる
    * v-bind:name=...とすることでトランジション名の制御や切り替えが可能
* トランジション用の属性を自由に切り替えられるだけでなく、イベントフックも切り替えられる
    * メソッドがデータに応じて振舞を変えられるのは当たり前
    * つまりJSトランジションを状態に応じて変えられる
